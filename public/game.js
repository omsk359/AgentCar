...
/******/ 	__webpack_require__.p = "http://debian359.tk/";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DEBUG = typeof localStorage != 'undefined' && !!localStorage.getItem('agentCarDebug');
	if (DEBUG) {
	    window.$ = $; // make jq available for debug
	}

	var scriptUrl = $(document.currentScript).attr('src');
	var urlParser = document.createElement('a');
	urlParser.href = scriptUrl;
	var DOMAIN = urlParser.host;

	var OWNER_ID = $(document.currentScript).data('id');
	var asteroid = new _asteroid2.default(DOMAIN, location.protocol == 'https:');
	...
	function sendSubscribe(result, contactInfo) {
	    var resultType = _helpers.resultTypes.indexOf(result);
	    if (resultType == -1) return console.error('Wrong result "' + result + ' (' + JSON.stringify(_helpers.resultTypes) + ')"');
	    DEBUG && console.log('Params: ', { OWNER_ID: OWNER_ID, resultType: resultType, contactInfo: contactInfo });
	    return asteroid.call('gameSubscribe', OWNER_ID, resultType, contactInfo).result.then(function (result) {
	...
	$(function () {
	    ...
	    $('#ac_game_ifr').on('load', function () {
	        ...
	        formFrame.on('load', function () {
	            ...
	            form.attr('action', '');
	            form.find('[type=submit]').attr('onclick', function () {});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_RESULT__;/* WEBPACK VAR INJECTION */(function(DDP, Q) {"use strict";
